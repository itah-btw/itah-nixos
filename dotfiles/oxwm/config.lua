...
local blocks = {
    ...
    oxwm.bar.block.shell({
        format = "CPU: {}",
        command = "f=/tmp/oxwm-cpu;p=$(cat $f 2>/dev/null);r=$(sed -n 1p /proc/stat);set -- $r;t=$(( $2+$3+$4+$5+$6+$7+$8+$9 ));i=$5;printf \"%s %s\" $t $i>$f;[ -n \"$p\" ]&&set -- $p && [ $((t-$1)) -gt 0 ]&&printf '%s%%' $((100-(i-$2)*100/(t-$1)))",
        interval = 2,
        color = colors.purple,
        underline = true,
    }),
    ...
    oxwm.bar.block.shell({
        format = "Temp: {}",
        command = "t=0; for z in /sys/class/thermal/thermal_zone*/type; do [ \"$(cat \"$z\")\" = \"x86_pkg_temp\" ] && t=$(cat \"${z%/type}/temp\"); done; [ \"$t\" -gt 0 ] && printf '%s°C' $((t/1000))",
        interval = 5,
        color = colors.red,
        underline = true,
    }),
...
-- Screenshot (select a region, saved to ~/Pictures). Shows a dunst popup
-- with a thumbnail action: left-click opens the image in imv.
oxwm.key.bind({ modkey }, "S", oxwm.spawn({ "sh", "-c", "f=\"$HOME/Pictures/$(date +%Y-%m-%d-%H%M%S).png\"; mkdir -p \"$HOME/Pictures\" && scrot -s \"$f\" && dunstify -a screenshot -r 3003 -A \"default,imv '$f'\" \"Screenshot saved\" \"$f\"" }))
-- Clipboard history picker (requires the clipmenud daemon). Notifies with a
-- preview of what was copied to the clipboard.
oxwm.key.bind({ modkey }, "V", oxwm.spawn({ "sh", "-c", "s=$(CM_OUTPUT_CLIP=1 clipmenu -l 10); if [ -n \"$s\" ]; then d=$(printf '%s' \"$s\" | awk 'NR==1{sub(/\\r$/,\"\"); print substr($0,1,80)}'); dunstify -a clipmenu -r 3004 \"Copied to clipboard\" \"$d\"; fi" }))
